...
So now we have a value and we want to add it to the tree. The value is added in the form of a node, and in this case, it's going to become the root node. Say we add a node with the value `17`. (I won't discuss [applications of the BST](http://shelbyspees.github.io/speesblog/data_structures/2015/03/11/finals-bag-adt.html#bst-bag) here, but assume we have a good reason for doing so.)
